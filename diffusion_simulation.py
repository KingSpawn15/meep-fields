...
def solve_diffusion(n_e_0, params):
    ...
    output_dir = os.path.join("current_densities", "eps12", "diffusion", "saturated")
    os.makedirs(output_dir, exist_ok=True)

    sim_plot.create_gif_diffusion(
        params,
        gif_path=os.path.join(output_dir, f"diffusion_nc_{nc_formatted}_sigma_{sigma_y_formatted}.gif"),
        xmax=5
    )

    np.save(
        os.path.join(output_dir, f"currents_nc_{nc_formatted}_sigma_{sigma_y_formatted}.npy"),
...
if __name__ == '__main__':
    ...
    for puslse_energy_values in [5, 10, 17.3, 30]:
